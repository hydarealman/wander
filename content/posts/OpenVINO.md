...
**OpenVINO** 是英特尔（Intel）开源的一套工具包，专门用于**加速 AI 模型的推理（Inference）**。

**它的核心职责：** 它**不负责**训练模型（那是 PyTorch 或 TensorFlow 的工作），它只负责**使用**模型。它能把你训练好的模型进行底层指令集的优化，让它在 Intel 的硬件（比如 CPU、集成显卡 iGPU、或者最新的 NPU）上跑得飞快。
...
4\.create\_infer\_request \- 创建推理请求
